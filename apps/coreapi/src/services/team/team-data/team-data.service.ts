...
import { CaseTeamReq, CaseUserInfoReq, CaseUserInfoRes, CaseUserReq, RoleListRes, TeamComboRes, TimeZoneRes, UserListRes, assignedUsersReq, assignedUsersRes, teamListResonce } from 'apps/coreapi/src/interfaces/team.interface';
...
export class TeamDataService {
    ...
    async getTeamcolor(): Promise<TeamcolorRes> {
        let res = await this.db.executeRef('teamcolors', {});
        if (res.success) {
            return res.data[0];
        } else {
            return { msg: -1, value: 'Failed to fetch', error: res.error }
