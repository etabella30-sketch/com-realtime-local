  ...
  async parseDataQueue(incomingBuffer: Buffer, currentSession: any, currentJob: CurrentJob, sessions: any): Promise<void> {
    ...
    try {
      ...
      for (let byte of modifiedBuffer) {
        ...
        try {
          id = String((Number((currentJob.lineCount || 0)) + 1))
        } catch (error) {
        }
        currentJob.lineBuffer[currentJob.lineCount] = [crTm, currentJob.crLine, currentJob.lineCount, null, null, null, id];
        // currentJob.lineBuffer[currentJob.lineCount][6] = 
      ...
      if (this.sessionService.CurrentJob.lineBuffer.length) {
        ...
        if (array.length) {
          ...
          try {
            ...
            await this.utilityService.saveSessionsToFile(this.sessionService.currentSessionid, 'activesession', sessions);
          } catch (error) {
