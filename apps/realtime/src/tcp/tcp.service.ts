...
@Injectable()
export class TcpService implements OnApplicationBootstrap {
    connectivity_LOG = [];
    timeReconnect = 3000
    ...
    public sessions = new Map<number, SessionRecord>();
                ...
                this.client.on('data', (data) => {
                    ...
                    if (sesid != this.sessionService.currentSessionid) {
                        ...
                        try {
                            ...
                            this.verifyTab.getAllCaseTabs(sesid);
                        } catch (error) {
    ...
    async onApplicationBootstrap() {
        ...
        if (isInit) {
            this.logger.fatal(`Reading Session Data ${this.sessionService.currentSessionid}`)
            try {
                const data = await this.utilityService.readData(this.sessionService.currentSessionid, 'allsession');
                // console.log(data);
                this.logger.warn(`Rec Total Objects ${Object.keys(data || {})?.length}`);
                this.logger.warn(`lineBuffer ${data?.lineBuffer?.length}`);
                this.logger.warn(`lineCount ${data?.lineCount}`);
                this.logger.warn(`crLine ${data?.crLine}`);

                if (data?.lineBuffer) {
                    this.sessionService.CurrentJob.lineBuffer = data?.lineBuffer || [];
                    this.sessionService.CurrentJob.lineCount = data?.lineCount || 0;
                    this.sessionService.CurrentJob.crLine = data?.crLine || [];

                    const detailGlobal = await this.utilityService.readData(this.sessionService.currentSessionid, 'globalbuffer');
                    this.logger.warn(`Global Buffer ${detailGlobal?.length}`)
                    if (detailGlobal?.length) {
                        this.sessionService.CurrentJob.globalBuffer = detailGlobal || [];
                    }
                    const sessions = await this.utilityService.loadSessionsFromFile(this.sessionService.currentSessionid, 'activesession');
                    if (sessions) {
                ...
                this.logger.error(`Error in session`, error)
