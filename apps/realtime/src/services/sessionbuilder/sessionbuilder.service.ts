...
export class SessionbuilderService implements OnModuleInit {
      ...
      try {
        ...
        try {
          if (res.data[0].length) {
            const obj = await this.getFilesCount(body.nSesid, res.data[0][0]["cProtocol"], res.data[0][0].nLines);

            res.data[0][0] = { ...res.data[0][0], maxNumber: obj.maxNumber, pageRes: obj.pageRes, secondLastRes: obj?.secondLastRes || null }
          }
        } catch (error) {
        }
    ...
    try {
      if (cProtocol == 'B') {
        ...
        if (this.session.currentSessionid == nSesid) {
          ...
          const pageDataLength = nLines || 25;
          if (feeddata && feeddata.length) {
            const maxNumber = Math.ceil(feeddata.length / pageDataLength);

            const pageData = this.getPageData(feeddata, maxNumber, pageDataLength);

            const obj = { maxNumber, pageRes: pageData?.lenght ? JSON.stringify(pageData) : [], secondLastRes: null };

            try {
              if (maxNumber > 1) {
                const pageResY = this.getPageData(feeddata, maxNumber - 1, pageDataLength);
                if (pageResY?.length)
                  obj.secondLastRes = JSON.stringify(pageResY);
              }
            } catch (error) {
            }
            return obj
        ...
        const obj = { maxNumber, pageRes, secondLastRes: null };
        try {
          if (maxNumber > 1) {
            const filePathY = path.join(folderPath, `page_${maxNumber - 1}.json`);
            const pageResY = await this.processFile(filePathY);
            if (pageResY)
              obj.secondLastRes = pageResY;
          }
        } catch (error) {
        }
  ...
  async sessionCreation(body: SessionBuilderReq): Promise<any> {
    ...
    if (res.success) {
      try {
        ...
        if (obj.msg == 1) {
          await this.setupScheduler(obj.nSesid, parsedDateString);
        }
  ...
  async setupScheduler(nSesid: string, parsedDateString: string): Promise<void> {
    ...
    this.schedulerService.scheduleTask(nSesid, parsedDateString, async () => {
      this.logger.verbose(`Session Start task: ${nSesid}`);
      const mdl = {
        cStatus: 'R', nSesid
      }
  ...
  async sessionList(body): Promise<any> {
    const cSessionUnicId = await this.unicIdentity.getSessionUnicId();
    const res = await this.db.executeRef('sessions', { cSessionUnicId, nUserid: body.nUserid }, this.schema);
    if (res.success) {
      return res.data[0] || [];
    } else {
