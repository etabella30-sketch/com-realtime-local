...
import { CurrentJob, DocinfoReq, filedataReq, filedataRes } from '../interfaces/session.interface';
...
export class SessionService implements OnApplicationBootstrap {
    ...
    async getSessionsServers(): Promise<any> {
        try {

            const cSessionUnicId = await this.unicIdentity.getSessionUnicId();
            const res = await this.db.executeRef('session_servers', { cSessionUnicId }, this.schema);
            if (res.success) {
                try {
                    return res.data[0];
                } catch (error) {
