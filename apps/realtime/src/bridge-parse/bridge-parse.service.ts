...
export class BridgeParseService implements OnApplicationBootstrap {
  ...
  async performQuery(nSesid, pageno, cType, jIdentities) {
    const res = await this.db.executeRef('annottransfer_getdetail', { nSesid, pageno, cType, jIdentities: JSON.stringify(jIdentities.map(String)) }, 'realtime');
    if (res.success) {
      try {
        return res.data[0];
      } catch (error) {
        ...
        const tasks = changedAnnots.map((x) =>
          limit(async () => {
            ...
            try {
              ...
              await this.transferAnnotReport(`_Transfer`, 'annot', `"refreshCount":${this.sessionService.current_refresh},"nIDid": "${x.nIDid}","start": "${transferStartDetail}" \n `);
            } catch (error) {
            }
            ...
            if (!isNotInRefresh && !updateDetail) {
              affectedAnnots.push({ nIDid: x.nIDid, isInActivated: true, nUserid: x.nUserid });
              finalDbUpdate.push({ nIDid: x.nIDid, isInActivated: true, nUserid: x.nUserid });
            } else {
              if (updateDetail) {
            ...
            try {
              ...
              await this.transferAnnotReport(`_Transfer`, 'annot', `{"refreshCount":${this.sessionService.current_refresh}, "nIDid": "${x.nIDid}", "end": "${transferEndDetail}", "duration": ${x.transferEndTmDetail - x.transferStartTmDetail} },\n `);
            } catch (error) {
            }
...
*/
  ...
  async annotRefrshTransfer(affectedAnnots: any[], cType: 'A' | 'H') {
    try {
