...
import { SessionRecord } from '../tcp/tcp.service';
...
export class UtilityService implements OnApplicationBootstrap {
  ...
  async readData(currentSession: string, filename: string): Promise<any> {
    const filePath = `data/sessions${currentSession}/${filename}.json`;
    try {
      const fullPath = path.resolve(filePath);
  ...
  /** Save `this.sessions` to JSON (as an array of [key, value] entries). */
  async saveSessionsToFile(sessionId: string,filename:string, sessions): Promise<void> {
    try {
      const fullPath = path.resolve(`data/sessions${sessionId}/${filename}.json`);
      await fsp.mkdir(path.dirname(fullPath), { recursive: true });

      const payload: [number, SessionRecord][] = Array.from(sessions.entries());
      await fsp.writeFile(fullPath, JSON.stringify(payload, null, 2), 'utf8');
    } catch (err) {
      this.logger?.error?.('Error writing sessions file:', err as Error);
    }
  }

  /** Load sessions from JSON file and repopulate `this.sessions`. Returns the Map. */
  async loadSessionsFromFile(sessionId: string,filename:string): Promise<Map<number, SessionRecord>> {
    try {
      const fullPath = path.resolve(`data/sessions${sessionId}/${filename}.json`);
      const raw = await fsp.readFile(fullPath, 'utf8');

      const parsed = JSON.parse(raw) as [number | string, SessionRecord][] | Record<string, SessionRecord>;

      // Support either array-of-entries or plain object on disk
      const entries: [number, SessionRecord][] = Array.isArray(parsed)
        ? parsed.map(([k, v]) => [Number(k), v])
        : Object.entries(parsed).map(([k, v]) => [Number(k), v as SessionRecord]);

     const sessions = new Map<number, SessionRecord>(entries);
      return sessions;
    } catch (err: any) {
      if (err?.code === 'ENOENT') {
        // Missing file: start empty and return it
        const sessions = new Map<number, SessionRecord>();
        return sessions;
      }
      this.logger?.error?.('Error reading sessions file:', err as Error);
