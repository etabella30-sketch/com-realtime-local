...
export class FeedDataService {
  ...
  checkSessionExists(sessionId) {
    return this.manager.hasSession(Number(sessionId));
  }
    ...
    try {
      ...
      for (const sessionId in sessionMap) {
        for (const pageNumber in sessionMap[sessionId]) {
          this.manager.setPageData(Number(sessionId), Number(pageNumber), sessionMap[sessionId][pageNumber]);
        }
      }
  ...
  async getPage(sessionId, pageNumber): Promise<any[]> {
    try {
      if (this.manager.hasPage(Number(sessionId), Number(pageNumber))) {
        return this.manager.getPageData(Number(sessionId), Number(pageNumber)) || [];
      }
      // RETURN DATA FROM REDIS
      const pageData = JSON.parse(await this.db.getValue(`session:${sessionId}:${pageNumber}`) || '[]') || [];
      if (pageData?.length) {
        this.manager.setPageData(Number(sessionId), Number(pageNumber), pageData);
      }
      return pageData
    } catch (error) {
      console.log(error);
  ...
  async setPage(sessionId, pageNumber, Data: any[]): Promise<boolean> {
    try {
      this.manager.setPageData(Number(sessionId), Number(pageNumber), Data);
  ...
  async getSessionAllData(sessionId: number): Promise<any[]> {
    try {
  ...
  /*async readSessionData(sessionId: number): Promise<{ [page: number]: any[] }> {
    try {
      let Obj = {};
      if (this.manager.hasSession(Number(sessionId))) {
  ...
  // ON SESSION END
  async sessionEnd(sessionId: number): Promise<void> {
    try {
