...
export class SessionService implements OnApplicationBootstrap {
    ...
    async sessionDelete(body: SessionDeleteReq): Promise<any> {
        ...
        if (res.success) {
            ...
            try {
                this.schedulerService.cancelJob(String(res.data[0][0].nSesid));
                this.schedulerService.cancelJob(`END_${String(res.data[0][0].nSesid)}`);
                //     for (let x of res.data[0]) {
                //         this.setSchedular(x)
                //     }
            } catch (error) {
    ...
    sessionSchedular(mdl) {
        try {
            if (mdl.nSesid) {
                this.schedulerService.cancelJob(String(mdl.nSesid));
                this.schedulerService.cancelJob(`END_${String(mdl.nSesid)}`);
                ...
                const job2Id = this.schedulerService.scheduleTask(`END_${String(mdl.nSesid)}`, mdl.dEnddt, async () => {
                    try {
                ...
                if (listOfSessions && listOfSessions.length) {
                    for (let x of listOfSessions) {
                        if (x.cRStatus == 'C') {
                            ...
                            try {
                                this.schedulerService.cancelJob(String(x.nSesid));
                                this.schedulerService.cancelJob(`END_${String(x.nSesid)}`);
                            } catch (error) {
                            }
                            try {
